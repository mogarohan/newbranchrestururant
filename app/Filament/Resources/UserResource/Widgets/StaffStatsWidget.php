...
class StaffStatsWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        if (!$user->isSuperAdmin()) {
            ...
            // 1. Agar Restaurant Admin hai, toh uske pure restaurant ke users ka filter lagega
            if ($user->isRestaurantAdmin()) {
                $baseQuery->where('restaurant_id', $user->restaurant_id);
                $userLimit = $user->restaurant?->user_limits ?? 0;
            }
            // 2. Agar Branch Admin (ya Manager) hai, toh sirf uski specific branch ke users ka filter lagega
            elseif ($user->isBranchAdmin() || $user->isManager()) {
                $baseQuery->where('restaurant_id', $user->restaurant_id)
                    ->where('branch_id', $user->branch_id); // 👈 Yaha branch filter lagaya hai

                // Branch level limit (Agar aapne limits branch par lagayi hai toh update karein, warna restaurant limit hi dikhayega)
                $userLimit = $user->restaurant?->user_limits ?? 0;
            }
        }
