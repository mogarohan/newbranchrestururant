...
class MenuDashboardStats extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        // 👇 FIX 1: Accurate Category Count (Inherits Main + Own Branch)
        $totalCategories = Category::where('restaurant_id', $restaurantId)
            ...
            ->filter(function ($cat) use ($user) {
                // If it's a main category and viewed by a branch, check pivot
        ...
        // 👇 FIX 2: Accurate Menu Item Count (Inherits Main + Own Branch)
        $totalItems = MenuItem::where('restaurant_id', $restaurantId)
            ...
            ->filter(function ($item) use ($user) {
                // If it's a main item and viewed by a branch, check pivot
        ...
        $customCss = "
        <style>
            .fi-wi-stats-overview-stats-ctn { display: grid !important; grid-template-columns: repeat(1, minmax(0, 1fr)) !important; gap: 0.5rem !important; }
            @media (min-width: 768px) { .fi-wi-stats-overview-stats-ctn { grid-template-columns: repeat(2, minmax(0, 320px)) !important; } }
            .sa-stat-card { background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 1rem; padding: 1.25rem; position: relative; overflow: hidden; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3); transition: transform 0.2s, box-shadow 0.2s; display: flex; flex-direction: column; justify-content: space-between; z-index: 1; width: 100%; height: 100%; }
            .dark .sa-stat-card { background-color: #1e293b; border: 1px solid #334155; box-shadow: none; }
            .sa-stat-card:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(0, 0, 0, 0.08); }
            .watermark-icon { position: absolute; right: -10%; bottom: -15%; width: 140px; height: 140px; opacity: 0.1; transform: rotate(-15deg); z-index: -1; pointer-events: none; }
            .card-top-row { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem; }
            .main-icon-wrapper { padding: 0.75rem; border-radius: 12px; display: flex; align-items: center; justify-content: center; }
            .sa-stat-label { font-size: 0.75rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.05em; display: block; margin-bottom: 0.25rem; color: #64748b; }
            .sa-stat-value { font-family: 'Poppins', sans-serif; font-size: 2.25rem; font-weight: 700; line-height: 1.1 !important; display: block; color: #0f172a; }
            ...
            .sa-action-btn { display: inline-flex; align-items: center; gap: 4px; font-size: 0.75rem; font-weight: 600; padding: 6px 12px; border-radius: 6px; cursor: pointer; transition: 0.2s; border: 1px solid transparent; }
        ...
        // Visibility restriction removed! Both main and branch admins can add items.
        ...
        return [
            Stat::make('', new HtmlString($htmlCategories))->extraAttributes(['style' => 'padding: 0 !important; background: transparent !important; border: none !important; box-shadow: none !important;']),
            Stat::make('', new HtmlString($htmlItems))->extraAttributes(['style' => 'padding: 0 !important; background: transparent !important; border: none !important; box-shadow: none !important;']),
        ];
    }
}